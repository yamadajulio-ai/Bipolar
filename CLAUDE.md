...
- **Google OAuth native bridge (2026-04-20)**: fix do fluxo OAuth no app Capacitor. SFSafariViewController tem cookie jar isolado do WebView, então iron-session setada no callback era inútil. Bridge com HMAC token curto (2min, assinado com subkey derivada do SESSION_SECRET): botão passa `?native=1` → callback detecta cookie `google-login-native` → redireciona `suportebipolar://auth-success?token=<bridge>` (ou `auth-error?error=...`). Deep-link handler em `deep-links.ts` intercepta scheme, chama `Browser.close()` e força `window.location.href` pra `/api/auth/native-session?token=...` (router não atinge API route). Essa rota verifica HMAC, cria iron-session no WebView, redireciona `/hoje|/onboarding`. Se user já tem sessão no Safari (cookie jar prévio), `/api/auth/google-login` já emite bridge direto pra evitar stranding em Safari. Apple Sign-In inalterado — já usa plugin nativo. Arquivos: `src/lib/oauth-native-bridge.ts`, `src/app/api/auth/native-session/route.ts`, `src/app/api/auth/google-login/{route,callback}.ts`, `src/lib/capacitor/deep-links.ts`, `src/app/(auth)/{login,cadastro}/page.tsx`.
