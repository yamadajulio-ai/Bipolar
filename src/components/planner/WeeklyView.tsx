  ...
  return (
    <div>
      ...
      {googleConnected === false && (
        <Link
          href="/integracoes"
          className="mb-4 flex items-center gap-3 rounded-lg border border-warning-border bg-warning-bg-subtle px-4 py-3 no-underline transition-colors hover:bg-warning-bg-subtle/80"
        >
          ...
          <div>
            <p className="text-sm font-medium text-warning-fg">Conecte seu Google Agenda</p>
            <p className="text-xs text-warning-fg/80">Seus eventos aparecerão automaticamente no planejador.</p>
          </div>
        </Link>
      )}
      ...
      {/* Syncing indicator */}
      {syncing && (
        <div className="mb-4 flex items-center gap-2 rounded-lg border border-info-border bg-info-bg-subtle px-4 py-2.5">
          <svg className="h-4 w-4 animate-spin text-info-fg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          <span className="text-sm text-info-fg">Sincronizando com Google Agenda...</span>
        </div>
      )}

      {/* Sync error */}
      {syncError && !syncing && (
        <div className="mb-4 rounded-lg border border-danger-border bg-danger-bg-subtle px-4 py-2.5" role="alert">
          <p className="text-sm text-danger-fg">{syncError}</p>
          {needsReauth ? (
            <a href="/integracoes" className="mt-1 inline-block text-xs font-medium text-danger-fg underline hover:text-danger">
              Reconectar Google Agenda
            </a>
          ) : (
            <button
              onClick={handleManualSync}
              className="mt-1 text-xs font-medium text-danger-fg underline hover:text-danger"
            >
              Tentar novamente
            </button>
          )}
