...
export function AlertCard({ layer, reasons, actions, safety, syndrome }: Props) {
  ...
  return (
    <div className={`rounded-xl border ${borderColor} ${bgColor} p-5 shadow-sm`} role="status">
      ...
      {/* Reasons */}
      {reasons.length > 0 && (
        <div className={`rounded-lg ${reasonBg} p-3 mb-3`}>
          <ul className="space-y-1">
            {reasons.map((r, i) => (
              <li key={i} className={`text-xs ${reasonText}`}>
                {reasonToLabel(r)}
              </li>
            ))}
          </ul>
